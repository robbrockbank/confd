...
execute_test_suite() {
    ...
    # Run confd one-shot to make sure that works and then as a background process.
    echo "Running confd as background process"
    confd -confdir=/etc/calico/confd -onetime -log-level=debug >$LOGPATH/log1 2>&1 || true
    confd -confdir=/etc/calico/confd -log-level=debug >$LOGPATH/log2 2>&1 &
    CONFD_PID=$!
    ...
    confd -confdir=/etc/calico/confd -log-level=debug >$LOGPATH/log3 2>&1 &
    CONFD_PID=$!
    ps
...
# Run an individual test, applying a set of resources using calicoctl and verifying
# the templates generated by confd as a result.
run_individual_test() {
    testdir=$1
...
download_templates_from_calico() {
    ...
    #cd $repo_dir; git checkout master; git pull origin master; cd -
}
